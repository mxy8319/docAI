...
- 凡引用片段中的事实、数字、定义或原文措辞，句末或分句末必须标注角标 [1]、[2]（与片段标题中的编号一致）；一句可标多个 [1][2]。不要使用未提供的编号，也不要改用其它样式（如【1】、脚注^1）。
...
/** 从助手正文中收集 [n] 引用，只保留落在检索结果范围内的编号，映射为 Citation。 */
export function collectValidatedCitations(assistantText: string, results: SearchResult[]): Citation[] {
